    ...
    <body>
        ...
        <div class="container-fluid py-6 px-5">
            <div class="text-center mx-auto mb-5" style="max-width: 600px;">
                <h1 class="display-5 text-uppercase mb-4"><span class="text-primary"><h2><%=request.getParameter("election_name")%> ELECTION</h2></span></h1>
            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <%
                                        ...
                                        try {
                                            ResultSet rs5 = st5.executeQuery("SELECT * FROM add_election WHERE id = '" + eid + "'");
                                            rs5.next();
                                            String election_name = rs5.getString("election_name");
                                            ResultSet rt = st1.executeQuery("SELECT * FROM vote WHERE uid = '" + uid + "' AND election_name='"+election_name+"'  AND eid = '" + eid + "' ");
                                            if (rt.next() == true) {%>
                                    <div>
                                        <br><br>
                                        <h3>Already Voted</h3>
                                    ...
                                    <table id="naresh">
                                        ...
                                        <tbody>
                                            <%            try {
                                                    ResultSet rs = st.executeQuery("SELECT * FROM add_candidate WHERE election_name = '" + election_name + "'");
                                                    if (!rs.next()) {
                                                        response.sendRedirect("Elections.jsp?No_candidate_added");
                                                    } else {
                                                        do {
                                            %>
                                            <tr>
                                                ...
                                                <td><a href="vote.jsp?cid=<%= rs.getString("id")%>&eid=<%=eid%>" class="btn btn-link"><font size="3"><strong>Vote</strong></font></a><br></td>
                                            </tr>
                                            <%
                                                                } while (rs.next());
                                                            }
